...
public class AutofacTestInvoker : XunitTestInvoker
{
    ...
    protected override object CreateTestClass()
    {
        ...
        if (!TestMethod.IsStatic && !Aggregator.HasExceptions)
        {
            ...
            try
            {
                if (!CancellationTokenSource.IsCancellationRequested) Timer.Aggregate(() => testClass = _testLifetimeScope.Resolve(TestClass));
            }
            finally
            {
                if (!MessageBus.QueueMessage(new TestClassConstructionFinished(Test))) CancellationTokenSource.Cancel();
            }
        }
